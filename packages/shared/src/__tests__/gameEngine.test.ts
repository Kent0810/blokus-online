...
describe('gameEngine', () => {
  it('rotates pieces 0, 90, 180, and 270 degrees', () => {
    ...
    expect(rotatePiece(l4, 0)).toEqual([
      [0, 0],
      [1, 0],
      [2, 0],
      [2, 1],
    ]);
    expect(rotatePiece(l4, 1)).toEqual([
      [0, 0],
      [0, 1],
      [0, 2],
      [1, 0],
    ]);
    expect(rotatePiece(l4, 2)).toEqual([
      [0, 0],
      [0, 1],
      [1, 1],
      [2, 1],
    ]);
    expect(rotatePiece(l4, 3)).toEqual([
      [0, 2],
      [1, 0],
      [1, 1],
      [1, 2],
  ...
  it('flips pieces horizontally and normalizes them', () => {
    ...
    expect(flipPiece(piece)).toEqual([
      [0, 0],
      [1, 0],
      [1, 1],
    ]);
  ...
  it('validates moves including corner, overlap, edge touch, and bounds checks', () => {
    ...
    expect(
      validateMove(
        { playerId: 'p2', pieceId: 'I1', rotation: 0, flipped: false, row: 0, col: 0 },
        state,
      ),
    ).toEqual({
      valid: false,
    ...
    state = applyMove(
      { playerId: 'p2', pieceId: 'I1', rotation: 0, flipped: false, row: 0, col: 19 },
      state,
    );
    state = { ...state, currentPlayerIndex: 0 };
    expect(
      validateMove(
        { playerId: 'p1', pieceId: 'I2', rotation: 0, flipped: false, row: 0, col: 1 },
        state,
      ),
    ).toEqual({
      valid: false,
    ...
    expect(
      validateMove(
        { playerId: 'p1', pieceId: 'I2', rotation: 0, flipped: false, row: -1, col: 0 },
        state,
      ),
    ).toEqual({
      valid: false,
    ...
    expect(
      validateMove(
        { playerId: 'p1', pieceId: 'I2', rotation: 0, flipped: false, row: 2, col: 2 },
        state,
      ),
    ).toEqual({
      valid: false,
  ...
  it('applies moves immutably and updates score and pieces', () => {
    ...
    const next = applyMove(
      { playerId: 'p1', pieceId: 'I1', rotation: 0, flipped: false, row: 0, col: 0 },
      state,
    );
  ...
  it('skips players with no legal moves and finishes the game when everyone is blocked', () => {
    ...
    expect(Array.isArray(next.skippedPlayers) ? next.skippedPlayers.sort() : []).toEqual([
      'p1',
      'p2',
    ]);
  });
